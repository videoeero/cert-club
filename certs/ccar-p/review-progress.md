...
Professional Exam Guide v1.0 (Effective July 2026, Exam code: CCAR-P). Sources and bank
...
and difficulty calibration (`harden-domain-questions`). All repository bias guards pass cleanly.
...
- **Batches API deduplication**: Consolidated four overlapping Batches items into distinct competencies: `sda-008` (asynchronous workload choice), `eval-001` (terminal states: `succeeded`, `errored`, `canceled`, `expired`), `stake-010` (24-hr SLA commitments with 500k request chunking), and `sda-020` (paired with fast models).
